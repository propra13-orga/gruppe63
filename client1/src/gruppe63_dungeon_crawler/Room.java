...
public class Room extends JPanel {
	...
	public Room(int elementwidth, int elementheigth, int level, GameClient game) {
		super();
		Room.elementwidth = elementwidth;
		Room.elementheight = elementheigth;
		this.setBounds(0, 0, 600, 650);
	...
	public Component add(Player2 player2) {
		Component component = super.add(player2);
		this.player2 = player2;
		return component;
	}

	public void status(Player player, Player2 player2) {
		int collision = Environment(player.getX(), player.getY(), 35,
				35);
		switch (collision) {
		
		case 5:
						
			if (game.getDown() & Environment(player2.getX(), player2.getY(), 35,
					35)==5) {
				game.nextRoom();
			}
			
			if (game.getDown()) {player.incWinnerpoints();}
